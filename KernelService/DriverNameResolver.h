...
// 调用方(.c 文件)必须在 include 任何其他头文件之前先 include <ntifs.h>
#include <ntddk.h>
#include <wdf.h>
...
//
// 功能:
//   给定驱动的 ImageBase (从 PsLoadedModuleList 拿到的基址),
//   在内核对象管理器中查找对应的 DRIVER_OBJECT,通过比对
//   DriverObject->DriverStart == ImageBase,找到该驱动真实
//   的对象名(即 \Driver\<Name> 中的 <Name>,通常等于服务名)。
//
// 为什么要做这一步:
//   应用层从 PsLoadedModuleList 拿到的是文件名(如 "OpenArkDrv64.sys"),
//   但驱动在对象管理器里的名字是基于服务名创建的(如 "OpenArkDrv")。
...
// 在给定目录(\Driver 或 \FileSystem)中,按 ImageBase 查找驱动对象名
