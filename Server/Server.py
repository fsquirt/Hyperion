...
# SIPAEVENT 名称字典 - 完整从 wbcl.h 导入
SIPA_EVENT_NAMES: dict[int, str] = {
    ...
    0x00050009: "SIPAEVENT_HYPERVISOR_LAUNCH_TYPE",
    ...
    0x0005000E: "SIPAEVENT_CODEINTEGRITY",
...
def _sipa_tlvs(raw: bytes, pcr: int, idx: int) -> list[_SipaEv]:
    ...
    while pos + 8 <= len(raw):
        ...
        # All aggregation containers get recursively parsed
        if eid in (0x40010001, 0x40010003, 0x40010004, 0x40010005, 0x40010006, 0x000F0001):
...
def _feat_elam(ev, s):
    f = _Feat("Early Launch Anti-Malware (ELAM)")
    detected = False
    enabled = False
    evid = []

    # Check ELAM policy
    e_policy = _s1(s, 0x00090003)
    if e_policy:
        policy = e_policy.u32
        if policy in (1, 2):
            enabled = True
            detected = True
            desc = "Auto" if policy == 1 else "Force"
            evid.append(f"ELAM Policy: {desc} enabled [0x00090003=0x{policy:X}]")
        elif policy == 0:
            detected = True
            enabled = False
            evid.append("ELAM Policy: Disabled [0x00090003]")

    # Check if ELAM actually measured something
    e_measured = _s1(s, 0x00090004)
    if e_measured and e_measured.u8 == 1:
        detected = True
        enabled = True
        evid.append("ELAM: Drivers measured during boot [0x00090004]")

    # Check for ELAM aggregation container
    if any(e.eid == 0x40010003 for e in s):
        detected = True
        enabled = True
        evid.append("ELAM Aggregation V2 present [0x40010003]")

    if detected:
        f.status = _FS.ENABLED if enabled else _FS.DISABLED
    elif any(e.eid >= 0x00090000 and e.eid <= 0x00090004 for e in s):
        f.status = _FS.UNKNOWN
    else:
        f.status = _FS.NOT_MEASURED

    f.evidence = "; ".join(evid) if evid else "No ELAM events found"
    f.detail = f.evidence
    return f


def _feat_drtm(ev, s):
    f = _Feat("Dynamic Root of Trust for Measurement (DRTM)")
    detected = False
    enabled = False
    evid = []

    e_auth = _s1(s, 0x000C0001)
    if e_auth:
        state = e_auth.u32
        if state == 1:
            detected = True
            enabled = True
            evid.append("DRTM State: Authenticated success [0x000C0001=1]")
        else:
            detected = True
            if state == 0:
                evid.append("DRTM State: Not authenticated")
            else:
                evid.append(f"DRTM State: Authentication failed (code={state})")

    if any(e.eid == 0x000C0002 for e in s):
        detected = True
        e_smm = _s1(s, 0x000C0002)
        if e_smm:
            level = e_smm.u32
            evid.append(f"SMM Protection Level: {level} [0x000C0002]")

    if any(e.eid in (0x0005003B, 0x0005003C, 0x0005003D) for e in s):
        detected = True
        for eid in (0x0005003B, 0x0005003C, 0x0005003D):
            e = _s1(s, eid)
            if e:
                if eid == 0x0005003B:
                    evid.append("VSM DRTM key roll detected")
                elif eid == 0x0005003D:
                    cnt = e.u32
                    evid.append(f"VSM SRTM anti-rollback counter = {cnt}")

    if detected and enabled:
        f.status = _FS.ENABLED
    elif detected:
        f.status = _FS.DISABLED
    elif any(e.eid >= 0x000C0000 and e.eid <= 0x000C0004 for e in s):
        f.status = _FS.UNKNOWN
    else:
        f.status = _FS.NOT_MEASURED

    f.evidence = "; ".join(evid) if evid else "No DRTM events found"
    f.detail = f.evidence
    return f
