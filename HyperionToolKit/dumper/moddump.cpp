...
namespace das {
	...
	// Mini / Mifudump 模式去重表: 已 dump 的 PID
	...
	bool InitDumpDir()
	{
		...
		if (attr == INVALID_FILE_ATTRIBUTES) {
			if (!CreateDirectoryW(dir.c_str(), NULL)) {
				Out(L"[警告] 创建 dumpfile 目录失败: " + dir + L"\n");
				return false;
			}
		}
		else if (!(attr & FILE_ATTRIBUTE_DIRECTORY)) {
			Out(L"[警告] dumpfile 路径被文件占用: " + dir + L"\n");
			return false;
		}
	...
	bool InitFileDumpDir()
	{
		...
		if (attr == INVALID_FILE_ATTRIBUTES) {
			if (!CreateDirectoryW(dir.c_str(), NULL)) {
				Out(L"[警告] 创建 FileDump 目录失败: " + dir + L"\n");
				return false;
			}
		}
		else if (!(attr & FILE_ATTRIBUTE_DIRECTORY)) {
			Out(L"[警告] FileDump 路径被文件占用: " + dir + L"\n");
			return false;
		}
		...
		std::wstring& outDumpFile)
	{
		...
		// 从内存读取模块映像
		if (size == 0 || base == 0) {
			Out(L"  [dump] 模块基址/大小无效,跳过: " + modulePath + L"\n");
			return false;
		}
		...
		if (!ReadProcessMemory(hProcess, (LPCVOID)base, buf.data(), size, &bytesRead)
			|| bytesRead == 0) {
			Out(L"  [dump] ReadProcessMemory 失败: " + modulePath + L"\n");
			return false;
		}
		...
		if (hFile == INVALID_HANDLE_VALUE) {
			Out(L"  [dump] CreateFile 失败: " + dumpPath + L"\n");
			return false;
		}
		...
		if (!ok || written != bytesRead) {
			Out(L"  [dump] WriteFile 失败: " + dumpPath + L"\n");
			return false;
		}
		...
		std::wstring& outDumpFile)
	{
		...
		if (!ok) {
			...
			Out(L"  [dump] MiniDumpWriteDump (Mini) 失败: err=" + std::to_wstring(err) + L"\n");
		...
		std::wstring& outDumpFile)
	{
		...
		if (!ok) {
			DWORD err = GetLastError();
			Out(L"  [dump] MiniDumpWriteDump 失败: err=" + std::to_wstring(err) + L"\n");
			// 删除空文件
			DeleteFileW(dumpPath.c_str());
			return false;
		}
		...
		std::wstring& outCopyName, bool& outCopied)
	{
		...
		if (CopyFileExW(modulePath.c_str(), copyPath.c_str(), NULL, NULL, &cancel, 0)) {
			...
			Out(L"    [file] 已拷贝: FileDump\\" + copyName + L"\n");
		}
		else {
			DWORD err = GetLastError();
			Out(L"    [file] 拷贝失败: " + copyName + L" (err=" + std::to_wstring(err) + L")\n");
		}
	}
