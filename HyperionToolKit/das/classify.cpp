...
static std::wstring FormatIndexPrefix(size_t idx, bool psapiMode)
{
    ...
    ss << L"[" << std::setw(4) << (psapiMode ? L"----" : std::to_wstring(idx)) << L"] ";
    return ss.str();
}
                        ...
                        std::vector<std::wstring>& thirdPartyFilePaths)
{
    ...
    for (const auto& src : entries) {
        idx++;
        const std::wstring& fileName = src.name;
        const std::wstring& rawPath = src.rawPath;
        const std::wstring& driverObjName = src.objectName;
