...
namespace das {
	...
	struct LoadedDriverEntry {
		...
		unsigned long       ImageSize;         // 映像大小,单位字节
		...
		wchar_t             DriverObjectName[64]; // 真实驱动对象名,形如 \Driver\<Name>,通常=服务名
	...
	// outResp: 输出,响应头,含 ImageBase/ImageSize/BytesDumped
	// 返回 true 成功;false 失败,用 GetLastError() 查错误码
	...
	// 进程侧剥离这些权限: TERMINATE|CREATE_THREAD|VM_OPERATION|VM_READ|VM_WRITE|SUSPEND_RESUME;
	// 线程侧剥离这些权限: SUSPEND_RESUME|TERMINATE|SET_CONTEXT|GET_CONTEXT
	// 返回 true 成功;false 失败,用 GetLastError() 查错误码
	...
	// 指向目标进程的高危句柄,含 PROCESS_VM_READ|VM_WRITE|VM_OPERATION
	// 返回 true 成功;false 失败,用 GetLastError() 查错误码
