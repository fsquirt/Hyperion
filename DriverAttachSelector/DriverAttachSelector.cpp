...
// ═══════════════════════════════════════════════════════════════════════
//  main — 遍历 C:\Windows\System32\drivers\*.sys 全部分类(测试用)
// ═══════════════════════════════════════════════════════════════════════

int wmain() {
    SetConsoleOutputCP(CP_UTF8);

    const std::wstring driversDir = L"C:\\Windows\\System32\\drivers";
    std::wstring pattern = driversDir + L"\\*.sys";

    WIN32_FIND_DATAW fd = {};
    HANDLE hFind = FindFirstFileW(pattern.c_str(), &fd);
    if (hFind == INVALID_HANDLE_VALUE) {
        WriteOut(L"无法打开目录: " + driversDir + L"\n");
        return 1;
    }
    ...
    do {
        ...
        std::wstring filePath = driversDir + L"\\" + fd.cFileName;
        ...
        switch (result.klass) {
            ...
            case DriverClass::THIRD_PARTY_WHQL:
                countThirdParty++;
                thirdPartyList.push_back({fd.cFileName, result.vendorName});
                break;
            case DriverClass::UNTRUSTED:        countUntrusted++; break;
        }

        // 单条简要输出(避免刷屏,只输出文件名 + 分类 + 厂商)
        std::wostringstream line;
        line << L"[" << std::setw(4) << total << L"] "
             << std::left << std::setw(40) << fd.cFileName
